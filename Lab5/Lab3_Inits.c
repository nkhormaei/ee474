...
and implementation for initializing timer as an interrupt
and gpio ports for ultrasound functionality
 */
...
void Timer_Init(void) {
  ...
  TIMER1_TAMR_R |= 0x1; // Enable one shot timer mode
  TIMER1_TAILR_R = 60;// Load the value 60 into the GPTMTAILR to achieve a 10us blink rate using the 60 MHz oscillator
  
  TIMER0_CTL_R  &= ~0x1; // Disable the timer using the GPTMCTL register
  TIMER0_CFG_R = 0x0; //select 32-bit mode using the GPTMCFG register
  TIMER0_TAMR_R |= 0x12; // Configure the TACDIR bit of the GPTMTAMR register to count up and enable periodic mode
  TIMER0_TAILR_R = 6; // Load the value 6 into the GPTMTAILR to achieve a 1us blink rate using the 60 MHz 
  
  NVIC_EN0_R = (1<<19u); // enable interrupt at port 19
...
void Port_Init(void) {
  ...
  // PB2 is echo pin (input) and PE1 is trigger pin (output)

  GPIO_PORTE_DIR_R  |= 0x2;             // set PE1 to output 
  GPIO_PORTE_DEN_R |= 0x2;  // enable digital output on PE1
  GPIO_PORTE_AFSEL_R &= ~0x2;   // disable AFSEL  
  GPIO_PORTE_PCTL_R &= ~0xF0;  

  GPIO_PORTB_DIR_R  &= ~0x4;  // set PB2 to input
  GPIO_PORTB_DEN_R |= 0x4;  // enable digital output on PB 2
  GPIO_PORTB_AFSEL_R |= 0x4; // enable AFSEL       
  GPIO_PORTB_PCTL_R |= 0x300; // set mux to select T0CCP0

  // initialize PB2 for interrupts
  GPIO_PORTB_IM_R &= ~0x4; 
